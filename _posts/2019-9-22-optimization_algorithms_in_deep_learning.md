...
- 다시 미끄럼틀 위의 SGD를 상상해보자. 
    - 이제 우리 SGD는 관성을 힘으로 미끄럼틀 끝에서도 더 나아갈 수 있다.
    - 그리고 가파른 미끄럼틀에서 내려올 때면 관성이 그만큼 커져서 경로에 있는 작은 언덕 쯤은 쑥 넘어버릴 수 있을 것이다.<br/><br/>
...
- 위의 끔찍한 예시에서 SGD는 앞에 압정이 있는 것을 알면서도 속도를 멈추지 못했다.
    - NAG는 이런 문제를 해결하기 위해 나온 최적화 방법이다.
        - NAG는 앞을 미리 보고 현재 관성을 조절하여 업데이트 크기를 조절하여 이 문제를 해결한다.<br/><br/>
...
- 지금까지 SGD와 Momentum 그리고 NAG에 대해서 알아보았다.
    ...
    - 이것이 어떤 문제가 될까 싶겠지만, sparse한 데이터를 예시를 들어 알아보자.
        ...
        - 이 때 $w_2$는 그 동안 상대적으로 조금 업데이트됐기 때문에 수렴점까지 거리가 아직 많이 남은 상태이다.
            - 따라서 $w_2$는 다른 파라미터들보다 더 크게 업데이트를 해야 수렴지점으로 조금이라도 빨리 갈 수 있을 것이다.
    - 이런 이유로 파라미터가 업데이트된 빈도 수에 따라 업데이트 크기를 다르게 해야 할 필요가 있다.
